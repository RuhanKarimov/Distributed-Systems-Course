...
public class LatencyInjector {
    ...
    public void maybeSleep() {
        if (!enabled) return;
        long extra = (jitterMs > 0) ? ThreadLocalRandom.current().nextLong(0, jitterMs + 1) : 0;
        long total = delayMs + extra;
        if (total <= 0) return;
        try { Thread.sleep(total); } catch (InterruptedException e) { Thread.currentThread().interrupt(); }
    }
